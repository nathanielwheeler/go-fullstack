...
// WithValues is a functional option that will construct a new values service.
func WithValues() ServicesConfig {
	return func(s *Services) error {
		s.Values = NewValuesService(s.db)
		return nil
	}
}
...
func (s *Services) AutoMigrate() error {
	return s.db.AutoMigrate(&User{}, &Value{}).Error
}

// DestructiveReset will drop tables and call AutoMigrate
func (s *Services) DestructiveReset() error {
	err := s.db.DropTableIfExists(&User{}, &Value{}).Error
	if err != nil {
		return err
	}
